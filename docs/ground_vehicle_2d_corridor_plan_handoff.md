...
| 参数 | PX4 空中 | 小车建议初值 |
|---|---:|---:|
| Mini 轨道半径 | `80m` 左右 | `5~8m` |
| Mini 速度 | `8~10m/s` | `0.8~1.5m/s` |
| Carrier 速度上限 | `9~10m/s` | `1.0~1.8m/s` |
| 末端 corridor 长度 | `~150m` | `8~15m` |
...
mini_orbit_radius: 6.0
mini_orbit_speed: 1.0

carrier_approach_speed_limit: 1.2
carrier_tracking_speed_limit: 1.4
carrier_docking_speed_limit: 1.2
...
- Mini 绕 `R=6m` 圆完整一圈。
- Mini 速度 `1.0m/s`，最低不低于 `0.6m/s`。
- Carrier 最大速度 `1.3m/s`。
- 两车进入同一切线走廊。
- Carrier 在 Mini 前方，全程无越位。
- terminal straight length `< 15m`。
- 最小距离 `< 0.5m`。
