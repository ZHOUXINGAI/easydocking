...
if [ "$START_MODE" = "auto" ] || [ "$START_MODE" = "window" ]; then
  WAIT_WINDOW_CMD=(
    ...
    -p release_after_orbit_delay_sec:="${RELEASE_AFTER_ORBIT_DELAY_SEC:-2.0}"
    ...
    -p rear_entry_prediction_carrier_speed_mps:="${REAR_ENTRY_PREDICTION_CARRIER_SPEED_MPS:-10.0}"
    ...
    -p rear_entry_global_intercept_primary_score_threshold:="${REAR_ENTRY_GLOBAL_INTERCEPT_PRIMARY_SCORE_THRESHOLD:-6.6}"
    -p rear_entry_global_intercept_min_rel_distance_m:="${REAR_ENTRY_GLOBAL_INTERCEPT_MIN_REL_DISTANCE_M:-44.5}"
    -p rear_entry_global_intercept_min_speed_margin_mps:="${REAR_ENTRY_GLOBAL_INTERCEPT_MIN_SPEED_MARGIN_MPS:-1.45}"
    ...
    -p rear_entry_allow_orbit_progress_release:="${REAR_ENTRY_ALLOW_ORBIT_PROGRESS_RELEASE:-false}"
    -p rear_entry_min_orbit_progress_ratio:="${REAR_ENTRY_MIN_ORBIT_PROGRESS_RATIO:-1.0}"
    -p rear_entry_enable_energy_timing_gate:="${REAR_ENTRY_ENABLE_ENERGY_TIMING_GATE:-true}"
    -p rear_entry_energy_min_after_orbit_sec:="${REAR_ENTRY_ENERGY_MIN_AFTER_ORBIT_SEC:-28.0}"
    ...
    -p orbit_gate_ready_altitude:="${ORBIT_GATE_READY_ALTITUDE:-38.0}"
    -p orbit_gate_ready_radius_tolerance:="${ORBIT_GATE_READY_RADIUS_TOLERANCE:-6.0}"
    -p orbit_gate_min_valid_samples:="${ORBIT_GATE_MIN_VALID_SAMPLES:-20}"
    -p orbit_gate_required_laps:="${ORBIT_GATE_REQUIRED_LAPS:-1.0}"
    -p orbit_gate_min_accumulated_angle_deg:="${ORBIT_GATE_MIN_ACCUMULATED_ANGLE_DEG:-360.0}"
    -p orbit_gate_return_tolerance_deg:="${ORBIT_GATE_RETURN_TOLERANCE_DEG:-20.0}"
