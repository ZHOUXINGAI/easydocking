...
RUN_DIR="$ROOT_DIR/results/live_mock_fixed_wing_$(date +%Y%m%d_%H%M%S)"
...
CARRIER_MAX_ACCEL="${CARRIER_MAX_ACCEL:-3.2}"
...
cleanup_existing() {
  ...
  pkill -9 -f "/home/hw/PX4-Autopilot/build/px4_sitl_default/bin/px4 -i 1" 2>/dev/null || true
  pkill -9 -f "/home/hw/PX4-Autopilot/build/px4_sitl_default/bin/px4 -i 2" 2>/dev/null || true
}
...
echo "[1/2] Starting stable mock fixed-wing + carrier stack"
...
echo "[2/2] Ready"
echo "Stable mock fixed-wing mode is running."
echo "Fixed-wing: take off to ${MINI_TAKEOFF_ALTITUDE} m, then orbit at ${MINI_ORBIT_SPEED} m/s."
echo "Carrier: waits idle until START, max XY speed ${CARRIER_MAX_SPEED_XY} m/s."
echo "Runtime logs: $RUN_DIR"
echo "Start docking with:"
echo "./scripts/start_docking_command.sh"
echo "Real PX4 SIH chain is still available with:"
echo "./scripts/start_px4_sih_simulation.sh"
